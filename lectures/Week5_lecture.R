...
surveys %>%
  ...
  head() #Casewhen won't handle NA like how ifelse will
...
surveys %>% 
  mutate(hindfoot_cat = case_when(
    ...
    TRUE ~ "small" #True goes through the previous lines and tests if true or false. If true, then big, if false then go to next one. if 29 is true, then medium, if false go to next one. Very last condition
  )) %>% 
  select(hindfoot_length, hindfoot_cat) %>% 
  head(10)
